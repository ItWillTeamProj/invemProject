...
$(function(){
	...
	$("form[name=frmReply]").submit(function() {
		if($("#nonuserid").val() == '' || $("#nonuserid").val() == '닉네임' ){
			alert("작성자 명을 입력해주세요");
			$("#nonuserid").focus();
			return false;
		}
		if($("#pwd").val() == '' ||  $("#pwd").val() == '비밀번호'){
			alert("비밀번호를 입력해주세요");
			$("#pwd").focus();
			return false;
		}
...
<article>
	...
	<div style="margin-top: 15px;">
		...
		<div style="overflow: hidden;">
			...
			<a class="btn btn-danger pull-right" id="delete"
				style="float: right; margin-left: 5px">삭제</a> <a
				class="btn btn-info pull-right" id="edit" style="float: right;">수정</a>
		</div>
	</div>
	<%
		} else {
	...
	<div style="overflow: hidden; margin-top: 10px;">
		...
		%>
		<span style="float: left; margin-left: 30px"><%=vo.getUserid()%>
			| <%=vo.getRegdate()%></span> <span
			style="float: right; margin-right: 20px">조회 <%=vo.getViews()%>
			| 추천 <%=vo.getRecommend()%> | 댓글<%=replyCount%>
		</span>
		<%
			} else {
		%>
		<span style="float: left; margin-left: 30px"><%=vo.getNonuserid()%>(<%=vo.getIpaddress()%>)
			| <%=vo.getRegdate()%></span> <span
			style="float: right; margin-right: 20px">조회 <%=vo.getViews()%>
			| 추천 <%=vo.getRecommend()%> | 댓글<%=replyCount%>
		</span>
	...
	<hr style="border: 0; height: 2px; background: lightgray">
	<%
		if (delflag.equals("N")) {
	%>
	<div style="overflow: hidden;"><%=vo.getDescribe()%></div>
	<br>
	<%
		} else {
	%>
	...
	<div
		style="width: 100%; height: 200px; text-align: center; margin-top: 150px;">
		<a href="#"><img src="../images/good.png"
			style="max-width: 80px; max-height: 80px; text-align: center;"
			alt="추천" id="good" /></a> <a href="#"><img src="../images/bad.png"
			style="max-width: 80px; max-height: 80px; text-align: center;"
			alt="신고" id="bad" /></a>
	...
	<table style="width: 100%;" class="table table-hover table-sm">
		...
		<tr>
			...
			<td><%=rVo.getReply()%><span style="display: none;"
				id="groupNo"><%=rVo.getGroupno()%></span><span
				style="display: none;" id="replyNo"><%=rVo.getRep_no()%></span></td>
			<td style="text-align: center"><%=sdf.format(rVo.getRegdate())%>
				<span><a href="#" class="delReply"><img
						style="width: 20px; height: auto; margin-left: 10px"
						src="<%=request.getContextPath()%>/images/deleteIcon.png"></a></span></td>
		</tr>
		<%
			}
		%>
	...
	<form name="frmReply"
		action="<%=request.getContextPath()%>/board/reply_ok.gg"
		method="post">
		<div style="overflow: hidden;">
			<%
				if ("unknown".equals(userid) || userid == null || userid.isEmpty()) {
			%>
			<span style="float: left; width: 19%"> <input type="text"
				name="nonuserid" size="15" id="nonuserid"
				style="margin-left: 5px; margin-top: 10px" value="닉네임"
				onfocus="this.value=''"> <input type="text" name="pwd" id="pwd"
				size="15" style="margin-left: 5px; margin-top: 10px" value="비밀번호"
			...
			%>
			<input type="hidden" name="delflag" value="<%=delflag%>"> <input
				type="hidden" name="code" value="<%=code%>"> <input
				type="hidden" name="no" value="<%=Integer.parseInt(no)%>">
